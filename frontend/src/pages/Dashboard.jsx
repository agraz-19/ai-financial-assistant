...
function ScopeSelector({ scope, statementId, statements, onScopeChange, onStatementChange }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2">
      ...
      <button onClick={() => onScopeChange("statement")} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition ${scope === "statement" ? "bg-blue-600 text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
        <FileText size={16} />
        This Statement
      </button>
      {scope === "statement" && (
        <select value={statementId || ""} onChange={(event) => onStatementChange(event.target.value)} className="ml-auto rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500">
          {statements.map((statement) => <option key={statement.id} value={statement.id}>{statement.filename || `Statement #${statement.id}`}</option>)}
        </select>
      )}
    </div>
  );
